...
5. **Логи при сбое**  
   В дашборде Bothost или логах контейнера смотрите строки уровня **warning/error**: **`messages API: 401`** — токен или формат **`Authorization`** (попробуйте **`MAX_USE_BEARER=1`**); **`Не удалось извлечь chat_id`** — формат **Update** не совпал с ожидаемым (сверьте JSON с актуальной схемой на **dev.max.ru**); дубли «эхо» — ретраи вебхука (в коде есть заготовка идемпотентности по **`mid`**).  
   Если в подписке указан **`secret`**, в Bothost в env должен быть **тот же** **`WEBHOOK_SECRET`**, иначе заголовок **`X-Max-Bot-Api-Secret`** не совпадёт → **403**. Если **`WEBHOOK_SECRET`** в контейнере **не задан**, а в подписке MAX **`secret` есть**, платформа шлёт заголовок, но код не сравнивает его с пустым значением — проверьте, что в подписке **`secret`** либо совпадает с env, либо подписка без **`secret`** (для теста).
...
6. **Исходящий API (401)**  
   ...
   Ответ **`Unknown recipient`** на **`POST /messages`** часто значит, что для лички нужен **`user_id`** отправителя в **query**, а не `chat_id` из `recipient` в JSON тела — в актуальном `echo_bot.py` это учтено (см. [POST /messages](https://dev.max.ru/docs-api/methods/POST/messages) и `recipient.chat_type`: `dialog` / `chat` / `channel`).
